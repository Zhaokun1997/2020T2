...
def apply_stem_lancaster(line):
    wn = WordNetLemmatizer()
    ls = LancasterStemmer()
    line_words = line.split(' ')
    temp_words = []
    for word in line_words:
        temp_words.append(wn.lemmatize(ls.stem(word)))
    result_line = ' '.join(temp_words)
    return result_line
...
def contents_pre_processing_with_stopwords_stem(contents):
    ...
    for line in contents:
        ...
        line_stem_words = apply_stem_words(line_rm_junk)
        # line_lancaster_words = apply_stem_lancaster(line_stem_words)
...
def create_count_vectorizer(train_X, test_X):
    ...
    count = CountVectorizer(token_pattern='[a-zA-Z0-9@#$_%]{2,}', max_features=1400, lowercase=True)
...
def print_report():
    ...
    # print(BNB_model.predict_proba(test_X_bag_of_words))
...
if __name__ == '__main__':
    ...
    BNB_model = clf.fit(train_X_bag_of_words, valid_train_y)
