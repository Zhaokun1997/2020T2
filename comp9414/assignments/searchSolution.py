...
def show_combinations(problem, name):
    ...
    for pruning in [None, 'cycle', 'mpp']:
        for method in ['astar', 'best', 'least-cost']:
            ...
            path = s.search()
            if path:
                print(method, "with", pruning, "expanded",
                      s.num_expanded, "paths. Solution cost:", path.cost)
            else:
                print(method, "with", pruning, "did not find a solution with",
                      s.num_expanded, "paths expanded")


if __name__ == "__main__":
    show_combinations(searchProblem.cyclic_delivery_problem,
                      "Cyclic Delivery Problem")
