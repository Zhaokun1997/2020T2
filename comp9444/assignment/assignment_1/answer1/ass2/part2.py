...
class NetworkLstm(tnn.Module):
    ...
    def forward(self, input, length):
        ...
        """
        #lstm_input = tnn.utils.rnn.pack_padded_sequence(input, length, batch_first=True)
        out, (hidden, c) = self.lstm(input)
...
def measures(outputs, labels):
    ...
    fp = 0
    fn = 0
    size = list(labels.shape)[0]
    for i in range(size):
        if labels[i] == 1 and prediction[i] == 1:
            tp += 1
        elif labels[i] == 0 and prediction[i] == 0:
            tn += 1
