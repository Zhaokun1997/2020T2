...
class Network(tnn.Module):
    def __init__(self):
        ...
        dropout_rate = 0.15
        self.lstm = tnn.LSTM(input_size = 50, 
                             hidden_size = 100,
                             num_layers = 2,
                             batch_first=True, 
                             bidirectional = True,
        ...
        #self.dropout = tnn.Dropout(dropout_rate)
        self.fc2 = tnn.Linear(100, 1)
        #self.cnn = tnn.Conv1d(in_channels=200, out_channels=200, kernel_size=8)
        #self.maxpool1 = tnn.MaxPool1d(4)
        #self.maxpool2 = tnn.AdaptiveMaxPool1d(1)
...
class PreProcessing():
    def pre(x):
        ...
        for string in x:
            ...
            string = re.sub(r'[^a-z0-9A-Z@#_$%\s]','',string)
            new.append(string)
        return new
