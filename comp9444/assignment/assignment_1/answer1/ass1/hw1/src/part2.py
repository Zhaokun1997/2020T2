...
class LinearModel:
    ...
    def forward(self, inputs):
        ...
        w = self.weights[:-1]
        b = self.weights[-1]
        return self.activation(np.dot(w, inputs) + b)
    ...
    def loss(prediction, label):
        ...
        """
        if (prediction == 0):
            loss = -label * np.log(0.000000000000001) - (1 - label) * np.log(1)
        elif (prediction == 1):
            loss = -label * np.log(1) - (1 - label) * np.log(0.000000000000001)
        else:
    ...
    def backward(self, inputs, diff):
        ...
        w = self.weights[:-1]
        b = self.weights[-1]
        self.weights[:-1] = w + self.lr * inputs * diff
        self.weights[-1] = b + self.lr * diff
